...
router.post('/', (req, res) => {
    ...
    if ( !title || !contents){
        res.status( 400 ).json( { message: 'My guy....you are missing a title or a content.. give me something to work with.'})
    } else{
...
router.get('/', (req ,res ) =>{
    ...
    .then( post => {
        ...
        res.status(500).json({ error: err, message:"Could not secure the bag"})
    })
})
...
router.get('/:id', (req ,res ) =>{
   ...
   .then( id => {
       if(id){
           ...
           res.status(404).json( {message: " You see this Id? ... cuz I dont"})
       }
   }).catch( err => {
    res.status(500).json({ error: err, message:"Could not secure the bag"})
})
...
router.put('/:id', (req, res) => {
    ...
    .then( posts => {
        if(posts){
            ...
            res.status(404).json( {message: "nahh"})
        }
    })
    .catch( err => {
